...
#name = ARGS[1]
n    = parse(Int, ARGS[1])
eps_ = parse(Float64, ARGS[2])
@everywhere eps_val = $eps_
println("eps = ", eps_)
...
function generate_figure_gaussian_20d_data(max_time, n_repeats, opt_params,overwrite = false)
    ...
    filename = "gaussian-d20" * string(eps_) * ".csv"
...
if n <= 3
    ...
    a,b,name = PDMPs[n]
    filename = "params" * string(eps_) * name * ".txt"
    ...
    opt_params["BPS"]     = read_final_params("params" * string(eps_) * "BPS.txt")
    opt_params["CA-BPS"]  = read_final_params("params" * string(eps_) * "CA-BPS.txt")
    opt_params["SL-PDMP"] = read_final_params("params" * string(eps_) * "SL-PDMP.txt")
    print(opt_params)
    full_df = generate_figure_gaussian_20d_data(10. ,100,opt_params,false)
    ...
    CSV.write("gaussian-d20-ks" * string(eps_) * ".csv", df_filtered)
    ...
    savefig(plt,"test" * string(eps_) * ".png")
    display(plt)
    ...
    savefig(plt,"test2" * string(eps_) * ".png")
end